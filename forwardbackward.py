...
#############################################################################
# Log-Likelihood
def logLikelihood(alphaT):
    result = np.log( np.sum(alphaT) )
    return result
#["word","A"][][][]
def getAccuracy(testInput, output):
    correctCount = 0.0
    allCount = 0.0
    for row in range(len(testInput)):
        for col in range(len(testInput[row])):
            if testInput[row][col][1] == output[row][col][1]:
                correctCount += 1
            allCount += 1
...
def predict(wordIndex, PI, A, B):
    ...
    for row in range(len(wordIndex)):
        ...
        for col in range(len(wordIndex[row])):
            ...
            conditionalProb = Alpha[col] * Beta[col]
            #print("CondiProb")
            #print(conditionalProb)
            tagIndex[-1].append( np.argmax(conditionalProb) )

        # Compute the LOG-LIKELIHOOD
        logLike = logLikelihood(Alpha[-1])
        sumLogLikelihood += logLike
    averageLogLike = sumLogLikelihood / len(wordIndex)



    # empty word is occupied by unknown value
    #print(tagIndex)
    return tagIndex, averageLogLike
...
def metricOut(averageLog, accuracy):
    result = [""]*2
    result[0] = "Average Log-Likelihood: " + str(averageLog)
    result[1] = "Accuracy: " + str(accuracy)
    return result
...
if __name__ == '__main__':
    ...
    predictTag, averageLog = predict(wordIndex, Pi, A, B)

    print("log", averageLog)


    output = predictToFile(predictTag, testInput, indexToTag)
    accuracy = getAccuracy(testInput, output)
    #print(output)
    threeDListToFile(predictedFile, output)

    listToFile(metricFile, metricOut(averageLog, accuracy))
    print(metricOut(averageLog, accuracy))
