...
def singleAlpha(sentence, Alpha, t, j, PI, A, B):
    ...
    if t == 0:
        ...
        for k in range(t):
            #dynamic programming
            if Alpha[sentence[t-1], k] != 0:
                sum = Alpha[sentence[t-1], k] * A[k][j]
            else:
...
def singleBeta(sentence, Beta, t, j, PI, A, B):
    ...
    if t == T:
        ...
        for k in range(len(B)):
            #DP
            if Beta[sentence[t+1]][k] != 0:
                sum = Beta[sentence[t+1]][k] * A[j][k] * B[k][sentence[t+1]]
            else:
